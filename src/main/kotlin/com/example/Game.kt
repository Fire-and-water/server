...
@Serializable
data class GameStatus(var isOver : Boolean, var isTwoConnected : Boolean, var what : String)
...
class Game(var idGame : Id, idCreator: Int, private var level: Int, roleCreator: ROLE, outputCreator: PrintWriter) {
    ...
    var gameStatus : GameStatus = GameStatus(isOver = false, isTwoConnected = false, what = "game-status")
    ...
    fun connectSecondPlayer(idPlayer: Int, outputPlayer : PrintWriter) : ROLE {
    ...
    fun sendPlayersPositions() {
        ...
        sendAll(Json.encodeToString(Coordinates(Point(wat.posX, wat.posY), Point(fir.posX, fir.posY), "coordinates")))
    ...
    fun saveResultsToDB() {
        val time : Long = System.currentTimeMillis() - timeStart
        UsersDB().saveGameResults(idGame, firePlayer!!.id, waterPlayer!!.id, max(0, 10 - time / 10000), time, mainGame.level);
    }
